...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if 'CIFAR' in dataset_name or 'MNIST' in dataset_name:
        ...
        #     model = WideResNet(num_classes=num_classes, depth=28, widen_factor=2, initial_conv=[3, 1, 1])
        else:
        ...
        if 'VGG16' in model_name:
            ...
            model = resnet_picker(model_name, dataset_name)
        # elif model_name == 'WideNet':
        #     model = WideResNet(num_classes=num_classes, depth=28, widen_factor=2, initial_conv=[7, 2, 3])
        else:
